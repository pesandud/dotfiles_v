...
update_system() {
  ...
  if sudo proxychains -q pacman -Syu --noconfirm >> "${LOG_FILE}" 2>&1; then
...
install_base_tools() {
  ...
  if sudo proxychains -q pacman -S --needed --noconfirm git base-devel curl >> "${LOG_FILE}" 2>&1; then
...
install_pacman_packages() {
  ...
  if sudo proxychains -q pacman -S --needed --noconfirm "${PACMAN_PACKAGES[@]}" 2>&1 | tee -a "${LOG_FILE}"; then
...
cargo_fix() {
  ...
  if ! command -v rustup &> /dev/null; then
    ...
    if sudo proxychains -q pacman -S --needed --noconfirm rustup >> "${LOG_FILE}" 2>&1; then
...
configure_shells() {
  ...
  if [[ ${#shells_to_install[@]} -gt 0 ]]; then
    ...
    if sudo proxychains -q  pacman -S --needed --noconfirm "${shells_to_install[@]}" >> "${LOG_FILE}" 2>&1; then
...
set_default_shell() {
  ...
  if [[ -z "${selected_shell}" ]]; then
    ...
    if sudo proxychains -q pacman -S --needed --noconfirm "${shell_name}" >> "${LOG_FILE}" 2>&1; then
      selected_shell="$(command -v "${shell_name}")"
      if [[ -z "${selected_shell}" ]]; then
